...
func newAddCmd(client grpcclient.GophKeeperClient, cache store.SecretStore, sess *session.Session, enc *crypto.Encryptor, _ *clientconfig.Config) *cobra.Command {
	var (
		secretType string
		name       string
		meta       string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			secret, err := client.CreateSecret(cmd.Context(), name, kind, encrypted)
			if err != nil {
